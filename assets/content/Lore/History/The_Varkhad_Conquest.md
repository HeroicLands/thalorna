---
tags:
  - history
  - spine
description: "Around 1400 BF the Varkhad come out of the further east into the Pelwar grasslands as conquerors, take the country and then merge into it—and the peoples who hold the north today carry both stocks."
type: lore
subType: history
name:
  full: The Varkhad Conquest
  aliases:
    - The Taking of the Pelwar Grasslands
shortcode: varkhadcnq
packFolder: settinglore
event:
  kind: conquest
  depth: world
  when:
    year: -1400
    precision: century
  sources:
    - lore-pelwarpepl
  summary: >-
    A fierce people out of the further east, the Varkhad, arrive in the eastern grasslands as
    conquerors and take the Pelwar homelands. They exterminate nobody: over generations they marry
    into the tribes they have beaten, adopt a good deal of what they find, and merge until the two
    stocks cannot be told apart. The peoples that come out of the mixture hold the north and
    north-east to this day.
  standing: single-source
  where:
    locus:
      - place-ankrscntnnt
    reach:
      - place: place-vrystwald
        how: the Varokh who hold this forest carry the double ancestry, and the temperament that came with the eastern half of it
        knowledge: named
      - place: place-nrdlndsrgn
        how: the Nordmen carry the same double ancestry, and refuse the debt of it outright
        knowledge: named
  who:
    - ref: lore-pelwarpepl
      role: victim
  accounts:
    - by: lore-pelwarpepl
      says: >-
        Nothing as an event. It happened to particular tribes over generations and the tradition kept
        the flight rather than the cause.
      agrees: silent
  unresolved:
    - where the Pelwar homelands lay, given only as the central or eastern interior of the continent
    - where the Varkhad came from, given only as the further east
---

The Pelwar of the eastern grasslands were, by every account that survives, a comparatively peaceful
people—herders and cultivators, tribal but not warlike, organized for grazing rather than for
conquest. Into that country, out of the further east, came the **Varkhad**.

They did not exterminate anybody. Over generations they took the country and then merged into it,
marrying into the tribes they had beaten and adopting a good deal of what they found, until the two
stocks could no longer be told apart. What came out of that mixture are the **Varokh** of
[[place-vrystwald|Vrystwald]] and the **Nordmen** of the [[place-nrdlndsrgn|Nordlands]], who share the
double ancestry and, notably, the temperament that came with the eastern half of it.

The [[lore-pelwarpepl|Pelwar]] who did not stay to be conquered went west and south, and that is
[[lore-pelwarmigr|the Migration]]. The displacement begins here.
