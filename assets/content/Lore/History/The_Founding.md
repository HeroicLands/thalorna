---
tags:
  - history
  - spine
description: "A Vylarian Senator backed by loyal legions and the newly chartered Ordo seizes supreme power and declares himself Emperor — Year 1 of the calendar most of western Ankaris still counts by."
type: lore
subType: history
name:
  full: The Founding
  aliases:
    - Year One
    - The Making of the Vylarian Empire
shortcode: thefounding
packFolder: settinglore
event:
  kind: accession
  depth: world
  when:
    year: 1
    precision: year
  sources:
    - affiliation-vylarinmpr
    - place-ankrscntnnt
  summary: >-
    A Vylarian Senator, backed by loyal legions and by the Ordo Arcanis, seizes supreme power and
    declares himself Emperor. The Senate survives and is reduced from a governing body to an advisory
    one. It is not a sudden revolution but the culmination of decades of erosion; the Emperor
    formalises what has already become reality. Western Ankaris reckons its years from this moment,
    and everything before it is dated Before the Founding.
  standing: attested
  names:
    - name: Year 1
      by: affiliation-vylarinmpr
      gloss: the epoch of the Common Calendar, from which BF and AF are counted
  where:
    locus:
      - place-vylariargn
    reach:
      - place: place-midhalnrgn
        how: >-
          the Senate that could revoke a charter is reduced to advising the man who holds the legions,
          and the provinces answer to him
        knowledge: named
      - place: place-helionis
        how: >-
          the Ordo's charter passes to an Augustar who owes it a debt, rather than to a Senate that
          could vote it away
        knowledge: named
      - place: place-aureldirgn
        how: >-
          every date written in these realms is counted from this year, including in the ones that
          left the Empire centuries ago
        knowledge: named
      - place: place-haradregin
        how: >-
          the calendar arrives with the legions and stays after them, which is the visible measure of
          where the Empire's cultural gravity still reaches
        knowledge: named
      - place: place-aelwyth
        how: >-
          an island that never saw a legion counts its years by a Vylarian Senator's coup, because
          its neighbours across the water do
        knowledge: named
  who:
    - ref: affiliation-vylarinmpr
      role: actor
    - ref: affiliation-ordoarcanis
      role: instrument
  follows:
    - event: lore-vylarirpbl
      how: ended
      note: the Republic established six and a half centuries earlier ends in this year
    - event: lore-lexarcana
      how: enabled
      note: the Ordo backed him, a Senate that could revoke its charter being more dangerous than one man who would owe it
  accounts:
    - by: place-ankrscntnnt
      says: >-
        The calendar is a product of Vylarian political authority and not a natural feature of the
        world, and its spread is a measure of where that authority reached.
      agrees: partly
    - by: affiliation-empirtkhpr
      says: >-
        Nothing of it. The Kheperi count from the Sep Tepy and give the western year as a conversion
        when they give it at all.
      agrees: silent
    - by: affiliation-tanvurempr
      says: Nothing of it. The empire keeps its own reckoning and has since before this one existed.
      agrees: silent
  unresolved:
    - the Senator's name, which no record gives
---

In what is now reckoned as **Year 1**, a powerful Senator — backed by loyal legions and by the newly
chartered [[affiliation-ordoarcanis|Ordo Arcanis]] — seized supreme power and declared himself
Emperor. The Senate survived but was reduced from a governing body to an advisory one.

The transformation was not a sudden revolution but the culmination of decades of political erosion.
The late Republic had been wracked by civil wars, factional violence and the dangerous concentration
of power in the hands of successful generals; the Emperor simply formalised what had already become
reality.

The calendar is the part of it that outlived the Empire. Events before this year are reckoned **Before
the Founding**, events after it **After the Founding**, and the reckoning is the working calendar of
Mídhalión, Aurèldía, Heliónis, Harad, Provènzia, Tarvénia, Élavendre and Aelwyth — including the
provinces that have long since gone their own way. Beyond that sphere other civilisations keep their
own, and the Common Calendar's reach is itself a visible measure of where the Empire's cultural
gravity still holds even where its legions no longer do.

The record does not give the Senator's name.
